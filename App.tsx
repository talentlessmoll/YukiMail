...
const App: React.FC = () => {
  ...
  const [config, setConfig] = useState<User['config']>({
    ...
    resendFrom: localStorage.getItem('resendFrom') || 'hello@yukimail.ai'
  });
  ...
  const addRecipients = (input: string) => {
    ...
    setRecipients(prev => [...prev, ...emails.map(email => ({ id: Math.random().toString(36).substring(2, 11), email, status: 'pending' as const }))]);
  };
      ...
      <div className="min-h-dvh flex items-center justify-center bg-[#fafafa] p-4 md:p-6 overflow-hidden">
        <div className="max-w-md w-full rounded-[40px] p-8 md:p-14 shadow-[0_32px_80px_-16px_rgba(0,0,0,0.1)] border border-white animate-yuki-up bg-white relative">
          ...
          <div className="flex flex-col items-center mb-10 text-center relative z-10">
            ...
            <p className="text-slate-400 font-medium text-xs md:text-sm uppercase tracking-[0.2em]">High Volume Delivery</p>
          </div>
          ...
          <div className="space-y-4 relative z-10">
            ...
            <div className="flex items-center gap-4 py-4 md:py-6">
              ...
              <span className="text-[9px] md:text-[10px] font-black text-slate-300 uppercase tracking-[0.3em]">Saved Gateways</span>
              <div className="h-px bg-slate-100 flex-1"></div>
            </div>
            ...
            <div className="space-y-2 max-h-40 overflow-y-auto pr-1 custom-scrollbar">
              {emailjsAccounts.length === 0 ? (
                <div className="text-center p-6 border-2 border-dashed border-slate-100 rounded-[20px]">
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest leading-relaxed">No custom nodes saved.<br/>Configure in Gateways.</p>
                </div>
              ) : (
          ...
          <div className="fixed inset-0 z-[100] bg-slate-900/40 backdrop-blur-xl flex items-center justify-center animate-fade">
            <div className="bg-white w-full h-full md:h-auto md:max-h-[90dvh] md:max-w-5xl md:rounded-[40px] shadow-2xl animate-yuki-up overflow-hidden flex flex-col md:flex-row border border-slate-200">
              <div className="w-full md:w-5/12 bg-slate-50 p-8 md:p-12 border-b md:border-b-0 md:border-r border-slate-100 overflow-y-auto custom-scrollbar">
                <div className="flex items-center justify-between md:block mb-8 md:mb-10">
                  <div className="flex items-center gap-3">
                    ...
                    <h3 className="text-xl font-black uppercase tracking-tight">Active Nodes</h3>
                  </div>
                ...
                <div className="space-y-4">
                  {emailjsAccounts.length === 0 ? (
                    <p className="text-xs font-bold text-slate-400 uppercase tracking-widest text-center py-10">Empty registry.</p>
                  ) : (
                    emailjsAccounts.map(acc => (
                      <div key={acc.id} className="bg-white p-4 rounded-[20px] border border-slate-200 shadow-sm flex items-center justify-between group animate-fade">
                        <div className="overflow-hidden">
                          ...
                          <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-0.5 truncate">{acc.serviceId}</p>
                        </div>
              ...
              <div className="flex-1 p-8 md:p-12 relative overflow-y-auto custom-scrollbar">
                ...
                <h2 className="text-2xl md:text-3xl font-black text-black mb-8 md:mb-12 uppercase tracking-tight">Configure Node</h2>
                ...
                <div className="space-y-6">
                  {activeProvider === 'google' && (
                    <div className="animate-fade">
                      <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-3 block">Google Client ID</label>
                      <input 
                        type="text" 
                        value={config.googleClientId} 
                        onChange={e => saveConfig({ googleClientId: e.target.value })} 
                        placeholder="OAuth Client ID" 
                        className="w-full p-4 md:p-5 rounded-[18px] bg-slate-50 border border-slate-100 focus:border-indigo-600 focus:bg-white outline-none transition-all font-bold text-sm" 
                      />
                    </div>
                  )}
                  {activeProvider === 'emailjs' && (
                    <div className="grid grid-cols-1 gap-5 bg-slate-50 p-6 md:p-8 rounded-[32px] border border-slate-100 shadow-inner animate-fade">
                      <div>
                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-2 block">Friendly Label</label>
                        <input type="text" value={newEjs.label} onChange={e => setNewEjs({...newEjs, label: e.target.value})} className="w-full p-4 rounded-[16px] bg-white border border-slate-100 focus:border-indigo-600 outline-none transition-all font-bold text-sm" placeholder="Support EmailJS" />
                      </div>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                          <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-2 block">Service ID</label>
                          <input type="text" value={newEjs.serviceId} onChange={e => setNewEjs({...newEjs, serviceId: e.target.value})} className="w-full p-4 rounded-[16px] bg-white border border-slate-100 focus:border-indigo-600 outline-none font-bold text-sm" />
                        </div>
                        <div>
                          <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-2 block">Template ID</label>
                          <input type="text" value={newEjs.templateId} onChange={e => setNewEjs({...newEjs, templateId: e.target.value})} className="w-full p-4 rounded-[16px] bg-white border border-slate-100 focus:border-indigo-600 outline-none font-bold text-sm" />
                        </div>
                      </div>
                      <div>
                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-2 block">Public Key</label>
                        <input type="text" value={newEjs.publicKey} onChange={e => setNewEjs({...newEjs, publicKey: e.target.value})} className="w-full p-4 rounded-[16px] bg-white border border-slate-100 focus:border-indigo-600 outline-none font-bold text-sm" />
                      </div>
                      <button onClick={addEmailJSAccount} className="w-full bg-indigo-600 text-white py-4 rounded-[16px] font-black text-[10px] uppercase tracking-widest shadow-xl hover:bg-indigo-700 active:scale-[0.98] transition-all flex items-center justify-center gap-3 touch-manipulation">
                        <PlusCircle size={18} /> Add Gateway Node
                      </button>
                    </div>
                  )}
                </div>
                <button onClick={() => setView('dashboard')} className="w-full mt-10 bg-black text-white py-5 rounded-[18px] font-black text-sm uppercase tracking-[0.2em] hover:bg-slate-800 transition-all shadow-2xl active:scale-[0.98] touch-manipulation">Apply Configuration</button>
              </div>
            </div>
          </div>
        )}
        ...
        <div className="p-8">
          <div className="bg-slate-50 rounded-[28px] p-6 border border-slate-100 group transition-all hover:bg-white hover:shadow-xl">
            <div className="flex items-center gap-4 mb-6">
              <img src={user.avatar} className="w-12 h-12 rounded-[14px] bg-slate-200 border border-white" alt="Avatar" />
            ...
            <button onClick={handleLogout} className="w-full flex items-center justify-center gap-3 py-4 rounded-[18px] bg-white text-slate-400 hover:text-red-500 hover:bg-red-50 hover:border-red-100 border border-transparent transition-all font-black text-[10px] uppercase tracking-widest active:scale-95">
          ...
          {view === 'dashboard' && (
            <div className="space-y-12 md:space-y-16 animate-yuki-up">
              <header className="text-center md:text-left">
                ...
                <p className="text-slate-400 font-bold text-base md:text-lg max-w-2xl mx-auto md:mx-0">Node operational via <span className="text-indigo-600 underline decoration-2 underline-offset-4">{user.provider.toUpperCase()}</span>.</p>
              </header>

              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 md:gap-8">
                <StatsCard icon={<Users className="text-indigo-600 w-5 h-5 md:w-6 md:h-6" />} label="Pool" value={recipients.length.toString()} />
                <StatsCard icon={<CheckCircle2 className="text-green-500 w-5 h-5 md:w-6 md:h-6" />} label="Success" value={successCount.toString()} />
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8">
                <button onClick={() => setView('recipients')} className="flex items-center gap-6 md:gap-8 p-6 md:p-10 rounded-[32px] md:rounded-[48px] bg-white border border-slate-100 hover:border-black transition-all group text-left shadow-[0_12px_32px_rgba(0,0,0,0.02)] active:scale-[0.98] touch-manipulation">
                  ...
                  <div>
                    <p className="text-xl md:text-3xl font-black text-black tracking-tight mb-0.5">New Sequence</p>
                    <p className="text-slate-400 font-bold text-[9px] md:text-xs uppercase tracking-widest">Targeted blast</p>
                  </div>
                </button>
                <button onClick={() => setView('settings')} className="flex items-center gap-6 md:gap-8 p-6 md:p-10 rounded-[32px] md:rounded-[48px] bg-white border border-slate-100 hover:border-indigo-600 transition-all group text-left shadow-[0_12px_32px_rgba(0,0,0,0.02)] active:scale-[0.98] touch-manipulation">
                   ...
                   <div>
                    ...
                    <p className="text-slate-400 font-bold text-[9px] md:text-xs uppercase tracking-widest">Network setup</p>
                  </div>
                </button>
              </div>
            </div>
          ...
          {(view === 'recipients' || view === 'compose') && (
            <div className="space-y-8 md:space-y-12 animate-yuki-up">
              <div className="flex items-center gap-3 text-[9px] md:text-[11px] font-black uppercase tracking-[0.3em] md:tracking-[0.5em] text-slate-200">
                ...
                <span className={`transition-colors ${view === 'compose' ? 'text-indigo-600' : ''}`}>Compose</span>
              </div>
              
              {view === 'recipients' ? (
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 md:gap-12">
                  <div className="lg:col-span-1 space-y-6">
                    <div className="bg-white rounded-[32px] md:rounded-[40px] p-6 md:p-10 border border-slate-100 shadow-xl">
                      <div className="flex items-center gap-3 mb-6 md:mb-8">
                         <div className="w-9 h-9 bg-slate-50 rounded-xl flex items-center justify-center border border-slate-100"><Target className="text-black w-4 h-4" /></div>
                         <h3 className="font-black text-lg md:text-xl uppercase tracking-tight">Email Pool</h3>
                      </div>
                      <textarea 
                      ...
                      />
                      <button onClick={() => setView('compose')} className="w-full mt-6 md:mt-10 bg-black text-white py-4 md:py-6 rounded-[20px] md:rounded-[24px] font-black text-[10px] md:text-xs uppercase tracking-[0.2em] shadow-xl hover:bg-slate-800 transition-all flex items-center justify-center gap-3 active:scale-95 touch-manipulation">Define Message <ChevronRight size={18}/></button>
                    </div>
                  </div>
                  <div className="lg:col-span-2 bg-slate-50 rounded-[32px] md:rounded-[48px] border border-slate-100 flex flex-col min-h-[300px] md:min-h-[500px] shadow-inner overflow-hidden">
                    <div className="px-6 py-4 md:px-8 md:py-6 border-b border-white flex justify-between items-center bg-white/40">
                      <h3 className="font-black text-slate-400 text-[10px] md:text-xs uppercase tracking-[0.2em]">Validated Targets ({recipients.length})</h3>
                      <button onClick={() => setRecipients([])} className="text-[9px] md:text-[10px] font-black text-slate-300 hover:text-red-500 uppercase tracking-widest transition-colors active:scale-90">Clear All</button>
                    </div>
                    <div className="p-6 md:p-10 flex-1 grid grid-cols-1 md:grid-cols-2 gap-3 overflow-y-auto custom-scrollbar max-h-[400px] md:max-h-none">
                      {recipients.length === 0 ? (
                        <div className="col-span-full flex flex-col items-center justify-center py-10 md:py-20 opacity-10">
                          ...
                          <p className="font-black text-sm md:text-lg mt-4 uppercase tracking-widest">No entries</p>
                        </div>
                      ) : (
                        recipients.map(r => (
                          <div key={r.id} className="flex justify-between items-center p-4 md:p-6 bg-white rounded-[18px] md:rounded-[24px] border border-slate-100 shadow-sm animate-fade">
                            ...
                            <button onClick={() => setRecipients(recipients.filter(x => x.id !== r.id))} className="text-slate-200 hover:text-red-500 p-1 active:scale-90 touch-manipulation"><Trash2 size={16} /></button>
                          </div>
                        ))
                      )}
                    </div>
                   ...
                   <div className="lg:col-span-2 space-y-6 md:space-y-8">
                    <div className="bg-white rounded-[32px] md:rounded-[56px] p-6 md:p-12 border border-slate-100 shadow-2xl space-y-6 md:space-y-10">
                      <input 
                        ...
                        placeholder="Blast Subject Line"
                      ...
                      <textarea 
                        ...
                        placeholder="Write your native email content here..."
                  ...
                  <div className="lg:col-span-1 space-y-6 md:space-y-8">
                    <div className="bg-white rounded-[32px] md:rounded-[48px] p-8 md:p-10 border border-slate-100 shadow-xl space-y-6">
                      <div className="flex items-center gap-4">
                        ...
                        <h3 className="font-black text-xl md:text-2xl tracking-tight">Campaign Info</h3>
                      </div>
                      <div className="space-y-4">
                    ...
                    <div className="flex flex-col gap-4">
                      <button onClick={() => setView('recipients')} className="w-full bg-white text-slate-400 border border-slate-100 py-4 rounded-[16px] font-black text-[10px] uppercase tracking-widest flex items-center justify-center gap-2 touch-manipulation active:scale-95">
                        <ChevronLeft size={16} /> Edit Target List
                      </button>
          ...
          {view === 'sending' && (
            <div className="py-10 md:py-24 flex flex-col items-center max-w-5xl mx-auto px-4 animate-yuki-up">
              ...
              <h2 className="text-3xl md:text-7xl font-black text-black text-center tracking-tighter mb-2 md:mb-4 uppercase leading-none">
                {isSending ? "In Progress" : "Sequence Complete"}
              </h2>
              <p className="text-slate-400 font-black text-center text-[10px] md:text-sm mb-12 md:mb-20 uppercase tracking-[0.3em] md:tracking-[0.4em]">Active Node: {user.provider}</p>
                ...
                <div className="max-h-[350px] md:max-h-[500px] overflow-y-auto p-5 md:p-8 space-y-3 custom-scrollbar">
                  {recipients.map(r => (
                    <div key={r.id} className="flex flex-col p-4 md:p-6 rounded-[24px] md:rounded-[32px] border border-slate-50 bg-white hover:bg-slate-50/50 transition-all animate-fade">
                      ...
                      {r.error && <p className="text-[10px] font-bold text-red-500 mt-3 p-3 bg-red-50/50 rounded-[16px] border border-red-50 leading-relaxed uppercase tracking-wide break-words">{r.error}</p>}
                    </div>
                  ))}
                </div>
              </div>
              {!isSending && <button onClick={() => setView('dashboard')} className="mt-12 md:mt-20 bg-black text-white px-12 md:px-20 py-5 md:py-8 rounded-[20px] md:rounded-[32px] font-black text-xs md:text-sm uppercase tracking-[0.3em] shadow-2xl hover:bg-slate-800 active:scale-95 transition-all touch-manipulation">Return to Hub</button>}
            </div>
          )}
        </div>
      </main>
        ...
        <div className="lg:hidden fixed inset-0 z-[110] bg-slate-900/60 backdrop-blur-md animate-fade" onClick={() => setIsMobileMenuOpen(false)}>
          <div className="bg-white w-[80%] h-full p-8 flex flex-col shadow-2xl animate-yuki-up" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center mb-10">
              <span className="font-black text-xl uppercase tracking-tighter">Yuki Navigation</span>
              <button onClick={() => setIsMobileMenuOpen(false)} className="text-slate-300 p-2"><X size={28} /></button>
            </div>
            
            <div className="flex items-center gap-4 p-4 mb-8 bg-slate-50 rounded-[24px] border border-slate-100">
            ...
            <button onClick={handleLogout} className="w-full flex items-center justify-center gap-3 p-5 rounded-[20px] text-red-500 font-black border border-red-100 bg-red-50 uppercase text-[10px] tracking-widest active:scale-95 touch-manipulation"><LogOut size={18} /> Logout</button>
          </div>
        </div>
      )}
    </div>
...
const StatsCard: React.FC<{ icon: any; label: string; value: string }> = ({ icon, label, value }) => (
  <div className="bg-white p-4 md:p-8 rounded-[24px] md:rounded-[36px] border border-slate-50 shadow-[0_8px_20px_rgba(0,0,0,0.02)] flex items-center gap-3 md:gap-6 group hover:shadow-xl transition-all md:hover:translate-y-[-4px]">
    <div className="w-10 h-10 md:w-16 md:h-16 rounded-[14px] md:rounded-[22px] bg-slate-50 flex items-center justify-center flex-shrink-0 border border-slate-100 transition-colors group-hover:bg-white">{icon}</div>
    <div className="overflow-hidden">
